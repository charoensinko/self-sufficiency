...
## 2026-07-24 — ผังแปลง (M3) ใช้ SVG + pointer events แทน Konva.js

สเปกเสนอ Konva.js หรือ SVG — เลือก SVG เพราะไม่ต้องเพิ่ม dependency (~150KB)
และความต้องการจริงมีแค่ลากย้ายรูปทรงพื้นฐาน:
- viewBox ของ SVG ตั้งเป็นขนาดแปลงจริงในหน่วยเมตร → พิกัด = เมตรตรงๆ ไม่ต้องแปลงสเกล
- ลากย้ายด้วย pointer events + setPointerCapture (รองรับนิ้ว/เมาส์ในโค้ดชุดเดียว)
- ปรับขนาดโซนใช้ช่องกรอกตัวเลขแทน drag handle — แม่นกว่าและง่ายกว่าสำหรับ
  ผู้ใช้สูงวัยบนจอสัมผัส
- route ใช้ `/layout` ได้ (โฟลเดอร์ `app/layout/` เป็น segment ปกติ
  ไม่ชนกับไฟล์ convention `app/layout.tsx` — ตรวจกับ build แล้ว)
